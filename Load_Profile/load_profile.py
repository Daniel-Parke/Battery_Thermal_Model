...
def initialise_load(self):
    ...
    self.interopolated_load_profile = interpolate_load_profile(self.load_profile)
...
def interpolate_load_profile(load_profile_df: pl.DataFrame) -> pl.DataFrame:
    ...
    load_profile_df = load_profile_df.interpolate().filter(
        ...
        pl.col("Energy_Use_kWh_Base") / 60,      # Divide by 60 account for conversion from 1hr to 1 minute
        pl.col("Energy_Use_kWh") / 60, 
    )
